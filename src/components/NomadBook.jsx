      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"20px 16px 80px"}}>
        {tab==="notes"&&(<div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:18}}>
            <button onClick={()=>setFilterChapter("all")} style={{fontSize:12,fontWeight:600,padding:"5px 12px",borderRadius:20,cursor:"pointer",fontFamily:"inherit",background:filterChapter==="all"?C.accent:C.bg,color:filterChapter==="all"?"#fff":C.muted,border:`1px solid ${filterChapter==="all"?C.accent:C.border}`}}>Tous ({periodNotes.length})</button>
            {CHAPTERS.filter(c=>chapterCounts[c.id]).map(c=>(<button key={c.id} onClick={()=>setFilterChapter(c.id)} style={{fontSize:12,fontWeight:600,padding:"5px 12px",borderRadius:20,cursor:"pointer",fontFamily:"inherit",background:filterChapter===c.id?C.accent:C.bg,color:filterChapter===c.id?"#fff":C.muted,border:`1px solid ${filterChapter===c.id?C.accent:C.border}`}}>{c.emoji} {c.label} {chapterCounts[c.id]>1?`(${chapterCounts[c.id]})`:""}</button>))}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:6}}>
            ...
            {selectMode&&filteredNotes.length>0&&(<Btn onClick={()=>setSelected(new Set(filteredNotes.map(n=>n.id)))} variant="ghost" style={{fontSize:12,padding:"5px 12px"}}>Tout sélectionner</Btn>)}
          ...
          {filteredNotes.length===0?<div style={{textAlign:"center",color:C.subtle,padding:"60px 0",fontSize:15}}>Aucune note dans ce chapitre.<br/><span style={{fontSize:13}}>Appuie sur + Note.</span></div>:<div style={{display:"flex",flexDirection:"column",gap:8}}>{filteredNotes.map(note=>(<NoteCard key={note.id} note={note} onDelete={requestDelete} onEdit={editNote} selectMode={selectMode} selected={selected.has(note.id)} onToggleSelect={()=>{setSelected(prev=>{const next=new Set(prev);next.has(note.id)?next.delete(note.id):next.add(note.id);return next;});}}/>))}</div>}
        </div>)}
